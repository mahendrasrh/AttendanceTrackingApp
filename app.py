...
def admin_required(f):
    ...
    def decorated(*args, **kwargs):
        if request.current_user.get('role') != 'admin':
            app.logger.warning(f"Access denied for user {request.current_user.get('username')}: Admin required.")
...
def add_user():
    """
    Allows an institution admin to add new users (e.g., managers/employees) to their tenant, 
    now requiring the employee_id to link the login to the profile.
    """
    if client is None or users_collection is None:
        return jsonify({"error": "Database connection not established."}), 500
    ...
    # REQUIRED FIELDS NOW INCLUDE 'employee_id'
    required_fields = ['username', 'password', 'employee_id']
    if not all(field in data for field in required_fields):
        app.logger.warning(f"User addition failed: Missing required fields.")
        return jsonify({'message': 'Missing required fields: username, password, employee_id'}), 400

    new_username = data['username']
    new_password = data['password']
    new_employee_id = str(data['employee_id'])
    ...
    # 1. Check if the target employee profile exists
    if not employees_collection.find_one({"employee_id": new_employee_id, "tenant_id": tenant_id}):
        app.logger.warning(f"User creation failed: Employee ID {new_employee_id} not found in tenant {tenant_id}.")
        return jsonify({'message': f"Employee ID '{new_employee_id}' does not exist in this institution. Create the employee profile first."}), 404


    # 2. Check for username conflict within the tenant
    if users_collection.find_one({"username": new_username, "tenant_id": tenant_id}):
        app.logger.warning(f"User creation conflict in tenant {tenant_id}: Username '{new_username}' already exists.")
        return jsonify({'message': f"Username '{new_username}' is already taken in this institution."}), 409 
    
    # 3. Check if this employee ID already has a login account
    if users_collection.find_one({"employee_id": new_employee_id, "tenant_id": tenant_id}):
        app.logger.warning(f"User creation conflict: Employee ID {new_employee_id} already has a login account.")
        return jsonify({'message': f"Employee ID '{new_employee_id}' is already linked to a login account."}), 409 
    ...
    new_user_data = {
        ...
        "role": new_role,
        "tenant_id": tenant_id,
        "employee_id": new_employee_id # NEW LINKAGE FIELD
    }

    try:
        users_collection.insert_one(new_user_data)
        app.logger.info(f"New user '{new_username}' ({new_role}, Employee: {new_employee_id}) added by admin in tenant {tenant_id}.")
        
        # ALSO UPDATE THE LOGIN TOKEN TO INCLUDE EMPLOYEE ID
        
        return jsonify({
            "message": f"User '{new_username}' linked to Employee ID '{new_employee_id}' added successfully.",
            "username": new_username,
            "role": new_role,
            "employee_id": new_employee_id
        }), 201
    except Exception as e:
        app.logger.error(f"Error during user creation by admin in tenant {tenant_id}: {e}")
        return jsonify({"error": "Server error during user creation. Please try again."}), 500
...
def add_employee():
    """
    Allows an institution admin to add a new employee, including their reports_to_employee_ids (now a list).
    
    Requires: employee_id, name, position, business_unit.
    Optional: reports_to_employee_ids (list), contact_email.
    """
    ...
    required_fields = ['employee_id', 'name', 'position', 'business_unit']
    if not all(field in data for field in required_fields):
        app.logger.warning(f"Employee addition failed: Missing required fields.")
        return jsonify({'message': 'Missing required fields: employee_id, name, position, business_unit'}), 400

    tenant_id = request.current_user.get('tenant_id')
    employee_id = str(data['employee_id'])
    
    # NEW: Expecting a list or use an empty list if missing
    reports_to_ids = data.get('reports_to_employee_ids', []) 
    # Ensure it's treated as a list even if a single string was accidentally passed
    if isinstance(reports_to_ids, str):
        reports_to_ids = [reports_to_ids] 
        
    # --- Validation Checks ---
    
    # 1. Check for employee_id conflict within the tenant
    if employees_collection.find_one({"employee_id": employee_id, "tenant_id": tenant_id}):
        return jsonify({'message': f"Employee ID '{employee_id}' is already registered in this institution."}), 409 

    # 2. Check if ALL required approver IDs exist
    if reports_to_ids:
    ...
    new_employee_data = {
        ...
        "reports_to_employee_ids": reports_to_ids, # Hierarchy list
    ...
    try:
        ...
        app.logger.info(f"New employee '{data['name']}' (ID: {employee_id}) added. Reports to: {reports_to_ids}")
        return jsonify({
        ...
        app.logger.error(f"Error during employee creation: {e}")
        return jsonify({"error": "Server error during employee creation. Please try again."}), 500
